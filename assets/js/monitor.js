...
function renderTableGrouped(devices, telemetry) {
    ...
    // 1. Juntar y filtrar todos los logs de estado de todos los dispositivos
    devices.forEach(device => {
        telemetry
            .filter(t => t.deviceId == device.id && (t.type === 'status' || !t.type))
            .forEach(log => {
                // Le pegamos el nombre y el ícono al log para usarlo en la tabla
                allLogs.push({ ...log, deviceName: device.name, deviceIcon: device.icon });
            });
    });

    // 2. Ordenar todos los registros del más nuevo al más viejo
    allLogs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    
    // 3. Tomar solo los últimos 10 eventos
    const recentLogs = allLogs.slice(0, 10);

    // 4. Dibujar las filas en el orden correcto (Hora -> Dispositivo -> Evento -> Valor)
    recentLogs.forEach(log => {
        const isON = (log.value == 1);
        
        // Validación para evitar el error "Invalid Date"
        ...
        html += `
            <tr>
                ...
                <td><span class="text-info"><i class="bi bi-power"></i> Energía</span></td>
                <td>${isON ? '<span class="badge bg-success">ON</span>' : '<span class="badge bg-secondary">OFF</span>'}</td>
            </tr>`;
    });
